...
enum ECDSASignatureValidator {
    ...
    /// Loaded from the build-time embedded obfuscated key bytes generated by
    /// `Scripts/embed_ecdsa_key.sh`. In DEBUG builds, falls back to nil
    /// to allow testing without a real key.
    private static let defaultPublicKeyData: Data? = {
        #if DEBUG
        return nil
        #else
        return _embeddedPublicKeyBytes
        #endif
    }()
